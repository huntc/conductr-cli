...
def scale_cluster(args, ports):
    ...
    if args.nr_of_containers > len(running_containers):
        ...
        stop_nodes()  # FIXME: Where is this function?
    else:
...
def run_conductr_cmd(instance, container_name, cond0_ip, envs, image, log_level, ports, feature_names, conductr_roles):
    ...
    port_args = resolve_docker_run_port_args(ports, instance)
...
def resolve_docker_run_port_args(ports, instance):
    port_args = []
    for port in ports | CONDUCTR_PORTS:
        port_args.append('-p')
        port_args.append(map_port(instance, port))
    return port_args
